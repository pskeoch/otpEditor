...
$varlist = ["name"];
$qry = mysqlselect_encconv($mysqli,$varlist,"utf8","sections","parent_id",$langtitlid,"sort_order");
$tparsecarr = xmlbuild_mysqlsel($mysqli,$qry,[$langtitlid,"parent_id"]);
...
foreach($tparsecarr as $tparsec) {
	$mainnavxml->addChildWithCDATA('nav', $tparsec["name"]);
}
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
//Add gallery nav element (from languages table, using language id row already obtained)
///////////////////////////////////////////////////////////////////////////
$thisgallref = $tlangrow["gallery"];
$thisgallnoteref = $tlangrow["gallery_note"];
...
foreach($tparsecarr as $tparsec) {
	...
	if ($tparsec["name"] == "bbbbbbbbbbbbbbbbbbbbbbb" ){
		$tsecchild = $mainsecgrp->addChild('section');
		$tsecchild->addAttribute("contentPaneType",$contpanelkup[$tparsec["pane_type"]]);
		$tsecchild->addChildWithCDATA('name', $tparsec["name"]);
		$tsecchild->addChildWithCDATA('copy', "filler copy for now, need to fix");
	} else {
		$tsecchild = $mainsecgrp->addChild('section');
		$tsecchild->addAttribute("contentPaneType",$contpanelkup[$tparsec["pane_type"]]);
		$tsecchild->addChildWithCDATA('name', $tparsec["name"]);
		$tsecchild->addChildWithCDATA('copy', $tparsec["copy"]);
	...
	$varlist = ["name",
				"kind",
				"location"];
	$qry = mysqlselect_encconv($mysqli,$varlist,"utf8","links","parent_section_id",$tparsec["section_id"],"sort_order");
	$tparsecarr = sqlbuild_mysqlsel($mysqli,$qry,[$tparsec["section_id"],"parent_section_id"]);
	
	$tlinksxml = $tsecchild->addChild('links');
	
	foreach($tlinksarr as $tlink) {
		$tlinkxml = $tlinksxml->addChild('link');
		$tlinkxml->addAttribute("kind",$linkkindarr[$tlink["kind"]]);
		$tlinkxml->addAttribute("location",$tlink["location"]);
		$tlinkxml->addChildWithCDATA('name',replace_specchars($tlink["name"]));
	}
					}
				}
